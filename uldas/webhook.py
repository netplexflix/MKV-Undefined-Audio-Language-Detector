...
def _apply_path_map(value: str, path_from: str, path_to: str) -> str:
    ...
    remap is used when ULDAS sees media under a different mount point than
...
def _build_tokens(new_path: Path, old_path: Path,
                  path_from: str, path_to: str) -> dict:
    new_s = _apply_path_map(str(new_path), path_from, path_to)
    old_s = _apply_path_map(str(old_path), path_from, path_to)
...
def send_remux_webhook(config, new_path: Path, old_path: Path) -> None:
    """Fire the configured webhook after a file has been remuxed.
    ...
    tokens = _build_tokens(
        new_path, old_path,
        getattr(config, "webhook_path_from", "") or "",
        getattr(config, "webhook_path_to", "") or "",
    )

    url = _substitute(getattr(config, "webhook_url", "") or "", tokens).strip()
    if not url:
        return

    method = (getattr(config, "webhook_method", "POST") or "POST").upper()
    if method not in ("GET", "POST"):
        method = "POST"

    content_type = (getattr(config, "webhook_content_type", "form")
                    or "form").lower()
    body_tmpl = getattr(config, "webhook_body", "") or ""
    data = None
    headers = _parse_headers(getattr(config, "webhook_headers", []))
    ...
    auth = None
    user = getattr(config, "webhook_auth_user", "") or ""
    if user:
        auth = (user, getattr(config, "webhook_auth_pass", "") or "")

    try:
        timeout = int(getattr(config, "webhook_timeout_seconds", 10) or 10)
